...
if __name__ == "__main__":
    ...
    np.save(f"./{pred_path}/mnist-test-conv2", predictions)
    anomaly_scores = clf.score_samples(mnist_test_maps)
    np.save(f"./{scores_path}/mnist-test-conv2", anomaly_scores)

    predictions = clf.predict(notmnist_maps)
    np.save(f"./{pred_path}/notmnist-conv2", predictions)
    anomaly_scores = clf.score_samples(notmnist_maps)
    np.save(f"./{scores_path}/notmnist-conv2", anomaly_scores)
